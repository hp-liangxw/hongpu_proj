...
class ElPreCuter:
    ...
    def __init__(self):
        ...
        for tmpl in all_templs:
            ...
            for tmpimg in glob.glob(os.path.join(tempath, "{}/*.png".format(tmpl))):
                ...
                templimg = cv2.resize(templimg,
                                      (int(templimg.shape[1] / self._scale), int(templimg.shape[0] / self._scale)))
    ...
    def _get_corners_of_templ(self, img, tmpl):
        ...
        for tmpname, templimg in self.all_templs[tmpl].items():
            ...
            if tmpname == 'tl':
                ...
                min_x, min_y, max_x, max_y = img.shape[1] * (cols - 1) // cols, img.shape[0] * (rows - 1) // rows, \
    ...
    def get_precut_corner_by_contour(self, image_data):
        ...
        if not self._is_good_corners({'tl': [tl, ], 'tr': [tr, ], 'bl': [bl, ], 'br': [br, ]}, w * self._scale,
                                     h * self._scale):
            return None
...
def get_cut_line_none_precut(img, rows, cols):
    ...
    vertical = np.convolve(vertical, np.ones(conv_width_half * 2 + 1).astype('int'))[
               conv_width_half: -1 * conv_width_half]
...
def grid_cut(img_data, rows, cols, edge_removed):
    """
    按晶格切割大图，返回切割后的图像及其位置，注意返回的位置为晶格图像在大图片中的位置 坐标
