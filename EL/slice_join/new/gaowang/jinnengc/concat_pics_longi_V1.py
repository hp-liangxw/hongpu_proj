...
with open(os.path.join(os.path.dirname(__file__), 'jn_c_72.pkl'), 'rb') as f:
...
def _undistort(img, col):
    ...
    camera_parameter_pkl_path = os.path.join(path, 'jn_camera_all.pkl')
...
def get_unbiased_corners(img, row_symbol, col_num, n_pyrDown = 2):
    x_pad, y_pad = np.uint([50, 100] / np.power(2, n_pyrDown))#
    templ = GOLDEN_UNIT_DICT['%s%i' % (row_symbol, col_num)][y_pad:-y_pad, x_pad:-x_pad]
    corners = GOLDEN_CORNER_DICT['%s%i' % (row_symbol, col_num)]
    corners = corners / np.power(2, n_pyrDown - 1)
    
    img = _pyrDown(img, n_pyrDown)
    templ = _pyrDown(templ, n_pyrDown - 1)
    mt = cv2.matchTemplate(img.astype(np.uint8), templ.astype(np.uint8), cv2.TM_CCOEFF_NORMED) 
    print(templ.shape, img.shape)
...
def get_unbiased_corners_cp(img, row_symbol, col_num, n_pyrDown = 2):
    x_pad, y_pad = cp.uint(np.array([50, 100]) / np.power(2, n_pyrDown))#
    ...
    img = _pyrDown(img, n_pyrDown)
    img = cp.asarray(img)
    mt = cv2.matchTemplate(img.astype(cp.uint8), templ.astype(cp.uint8), cv2.TM_CCOEFF_NORMED) 
    minval, maxval, minloc, maxloc = cv2.minMaxLoc(mt)
    xbias, ybias = maxloc #

    xbias -= x_pad
    ybias -= y_pad
    unbiased_corners = corners + cp.array([xbias, ybias])
#     xs, ys = unbiased_corners.T
    if maxval < 0.8:#
        return corners * cp.power(2, n_pyrDown)
    return cp.asnumpy(unbiased_corners * cp.power(2, n_pyrDown))


def joint_image(img_list, row_symbol, module_type = 'full', piece_list = [3,3,3,3]):
    """
    去边并水平拼接图像
    ...
    for i in range(4):
        ...
        t3 = time.time()
        pers_transed_img = np.hstack((pers_transed_img, np.ones((h, 3))))
